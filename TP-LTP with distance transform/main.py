...
-the LTP file contains 3 methods (distance_transform, ltp_indexation and ltp query), the ltp_indexation is used in the
indexation phase and ltp_query is used in the query phase, the distance transform is called by the two methods.
...
-The distance transform is used as a similarity metric in this project

-the K-NN results of our implementation are stored in the K-NN Test results excel file

-the file results comparison.jpg contains a comparison of the results we obtained in the pre-processing phase with the
ones in the paper, and a comparison of the results obtained when applying the distance transform in our implementation
...
query_result = index_base.interrogate_index_base(img=img_query, gamma=gamma, alpha=alpha, tau=tau,
                                                  ltp_threshold=ltp_threshold, ltp_tau=6, nbr_result=nbr_results)
